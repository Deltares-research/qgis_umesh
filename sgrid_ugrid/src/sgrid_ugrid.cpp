...
SUGRID::SUGRID(QFileInfo filename, QProgressBar* pgBar)
{
    long ret_value = 1;
...
//------------------------------------------------------------------------------
long SUGRID::open(QFileInfo filename, QProgressBar* pgBar)
{
    long ret_value = -1;
    m_fname = filename;  // filename without path, just the name
    m_grid_file_name = filename.absoluteFilePath();  // filename with complete path
    ...
    if (status != NC_NOERR)
    {
        QMessageBox::critical(0, QString("Error"), QString("SUGRID::read()\n    Failed to open file: %1").arg(path_fname));
        return ret_value;
    }
...
//------------------------------------------------------------------------------
long SUGRID::read_global_attributes()
{
    long status;
...
#ifdef NATIVE_C
    fprintf(stderr, "SUGRID::read_global_attributes()\n");
#endif    
...
//------------------------------------------------------------------------------
long SUGRID::get_global_attribute_value(std::string att_name, std::string* att_value)
{
    long status = 1;  // error by default
    bool att_name_found = false;
    for (int i = 0; i < global_attributes->count; i++)
    {
        if (att_name_found) { break; }
...
//------------------------------------------------------------------------------
FILE_TYPE SUGRID::get_file_type()
{
    return m_ftype;
}
//------------------------------------------------------------------------------
QFileInfo SUGRID::get_filename()
{
    return m_grid_file_name;
}

//------------------------------------------------------------------------------
long SUGRID::read_ugrid_mesh()
{
    int ndims, nvars, natts, nunlimited;
    int unlimid;
    std::string cf_role;
...
#ifdef NATIVE_C
    fprintf(stderr, "SUGRID::read_mesh()\n");
#else
    m_pgBar->setValue(15);
#endif    
    ...
    for (int i_var = 0; i_var < nvars; i_var++)
    {
        ...
        //QMessageBox::warning(0, QString("Warning"), QString("SUGRID::get_grid_mapping()\nProgress: (%1, %2) %3").arg(i_var + 1).arg(nvars).arg(var_name));
#endif
        length = 0;
...
//------------------------------------------------------------------------------
long SUGRID::read_kiss_mesh()
{
    long status = 1;
    return status;
}
//------------------------------------------------------------------------------
long SUGRID::set_grid_mapping_epsg(long epsg, std::string epsg_code)
{
    long status = 0;
    m_mapping->epsg = epsg;
...
long SUGRID::read_ugrid_variables()
{
    // read the attributes of the variable  and the dimensions
#ifdef NATIVE_C
    fprintf(stderr, "SUGRID::read_variables()\n");
#endif    
    int ndims, nvars, natts, nunlimited;
    int status;
    ...
    for (int i_var = 0; i_var < nvars; i_var++)
    {
        ...
        if (status == NC_NOERR)
        {
            ...
            for (int j = 0; j < ndims; j++)
            {
                ...
                if (time_series[0].nr_times != 0 && QString(m_dim_names[var_dimids[j]].data()) == time_series[0].dim_name)
                {
            ...
            if (m_mesh_vars->variable[m_nr_mesh_var - 1]->dims.size() == 2)
            {
                ...
                for (int i = 0; i < m_mesh_vars->variable[m_nr_mesh_var - 1]->dims.size(); i++)
                {
                    ...
                    if (QString(m_mesh_vars->variable[m_nr_mesh_var - 1]->dim_names[i].data()) == time_series[0].dim_name)
                    {
            ...
            if (m_mesh_vars->variable[m_nr_mesh_var - 1]->dims.size() == 3)
            {
                ...
                if (contains_time_dimension)
                {
                    for (int i = 0; i < m_mesh_vars->variable[m_nr_mesh_var - 1]->dims.size(); i++)
                    {
                        ...
                        {
                            size_t nr_layers = m_map_dim[m_map_dim_name["zs_dim_layer"]];
                            m_mesh_vars->variable[m_nr_mesh_var - 1]->nr_hydro_layers = (int) nr_layers;
                        ...
                        {
                            size_t nr_layers = m_map_dim[m_map_dim_name["zs_dim_interface"]];
                            m_mesh_vars->variable[m_nr_mesh_var - 1]->nr_hydro_layers = (int) nr_layers;
                        ...
                        {
                            size_t nr_layers = m_map_dim[m_map_dim_name["zs_dim_bed_layer"]];
                            m_mesh_vars->variable[m_nr_mesh_var - 1]->nr_bed_layers = (int)nr_layers;
            ...
            else if (m_mesh_vars->variable[m_nr_mesh_var - 1]->dims.size() == 4)
            {
                ...
                // - xy-space dimension
                // - z-space (nbedlayers)
                QString qname(m_mesh_vars->variable[m_nr_mesh_var - 1]->var_name.data());
...
//------------------------------------------------------------------------------
long SUGRID::read_sgrid_variables()
{
    // read the attributes of the variable  and the dimensions
#ifdef NATIVE_C
    fprintf(stderr, "SUGRID::read_variables()\n");
...
//------------------------------------------------------------------------------
long SUGRID::read_kiss_variables()
{
    long status = 1;
    return status;
}
...
//------------------------------------------------------------------------------
struct _mesh_variable * SUGRID::get_variables()
{
   return this->m_mesh_vars;
}
//------------------------------------------------------------------------------
struct _mesh_contact * SUGRID::get_mesh_contact()
{
    return this->m_mesh_contact;
}
//------------------------------------------------------------------------------
struct _ntw_nodes * SUGRID::get_connection_nodes()
{
    return this->m_ntw_nodes;
}
//------------------------------------------------------------------------------
struct _ntw_edges * SUGRID::get_network_edges()
{
    return this->m_ntw_edges;
}
//------------------------------------------------------------------------------
struct _ntw_geom * SUGRID::get_network_geometry()
{
    return this->m_ntw_geom;
}
//------------------------------------------------------------------------------
struct _mesh1d * SUGRID::get_mesh_1d()
{
#ifdef NATIVE_C
    fprintf(stderr, "SUGRID::get_mesh1d()\n");
#endif    
    return m_mesh1d;
}
//------------------------------------------------------------------------------
struct _mesh2d * SUGRID::get_mesh_2d()
{
#ifdef NATIVE_C
    fprintf(stderr, "SUGRID::get_mesh2d()\n");
#endif    
    return m_mesh2d;
}
//------------------------------------------------------------------------------
struct _mesh1d_string ** SUGRID::get_mesh1d_string()
{
#ifdef NATIVE_C
    fprintf(stderr, "SUGRID::get_mesh2d()\n");
#endif    
...
//------------------------------------------------------------------------------
struct _mesh2d_string ** SUGRID::get_mesh2d_string()
{
#ifdef NATIVE_C
    fprintf(stderr, "SUGRID::get_mesh2d()\n");
#endif    
...
//------------------------------------------------------------------------------
struct _mesh_contact_string ** SUGRID::get_mesh_contact_string()
{
#ifdef NATIVE_C
    fprintf(stderr, "SUGRID::get_mesh2d()\n");
#endif    
...
//------------------------------------------------------------------------------
struct _mapping * SUGRID::get_grid_mapping()
{
    return m_mapping;
}
//------------------------------------------------------------------------------
long SUGRID::get_epsg()
{
    return m_mapping->epsg;
}
//------------------------------------------------------------------------------
struct _variable * SUGRID::get_var_by_std_name(struct _mesh_variable * vars, std::string mesh, std::string standard_name)
{
...
//------------------------------------------------------------------------------
DataValuesProvider2D<double> SUGRID::get_variable_values(const std::string var_name)
{
...
//------------------------------------------------------------------------------
DataValuesProvider2D<double> SUGRID::get_variable_values(const std::string var_name, int array_index)
...
#ifdef NATIVE_C
    fprintf(stderr, "SUGRID::get_variable_values()\n");
#endif    
...
//------------------------------------------------------------------------------
DataValuesProvider3D<double> SUGRID::get_variable_3d_values(const std::string var_name)
...
//------------------------------------------------------------------------------
DataValuesProvider4D<double> SUGRID::get_variable_4d_values(const std::string var_name)
        ...
        if (var_name == m_mesh_vars->variable[i]->var_name)  //var_name is a three dimensio
        {
            if (!m_mesh_vars->variable[i]->read)  // are the z_values already read
            {
                ...
                for (int j = 0; j < m_mesh_vars->variable[i]->dims.size(); j++)
                {
                    ...
                    {
                        dims[0] = (long) m_map_dim[m_map_dim_name["time"]];
                        dim_to[j] = 0;
                        if (j != 0)
                        {
                            correct_order = false;
                    ...
                    {
                        dims[1] = (long) m_map_dim[m_map_dim_name["zs_dim_bed_layer"]];
                        dim_to[j] = 1;
                        if (j != 1)
                        {
                            correct_order = false;
...
//==============================================================================
//
int SUGRID::read_mesh1d_attributes(struct _mesh1d_string* mesh1d_strings, int i_var, std::string var_name, int topology_dimension)
{
    int status = 1;
...
int SUGRID::read_mesh2d_attributes(struct _mesh2d_string* mesh2d_strings, int i_var, std::string var_name, int topology_dimension)
{
    int status = 1;
...
int SUGRID::determine_mesh1d_edge_length(struct _mesh1d* mesh1d, struct _ntw_edges* ntw_edges)
{
    ...
    return 1;  // there is a 1D mesh or network
}
int SUGRID::create_mesh1d_nodes(struct _mesh1d* mesh1d, struct _ntw_edges* ntw_edges, struct _ntw_geom* ntw_geom)
{
...
//------------------------------------------------------------------------------
std::vector<std::string> SUGRID::get_names(int ncid, std::string names, size_t count)
{
    int var_id;
    int status;
...
//------------------------------------------------------------------------------
std::vector<std::string> SUGRID::get_string_var(int ncid, std::string var_name)
{
    int varid;
    int ndims;
    int nr_names = 0;
...
//------------------------------------------------------------------------------
int SUGRID::read_variables_with_cf_role(int i_var, std::string var_name, std::string cf_role, int ndims, int* var_dimids)
{
    Q_UNUSED(var_dimids);
    ...
    if (cf_role == "mesh_topology")
    {
        ...
        if (topology_dimension == 2)  // it is a unstructured mesh
        {
            ...
            if (m_mesh2d_strings[nr_mesh2d - 1]->edge_node_connectivity == "")
            {
                ...
                int max_edges_per_face = (int) (m_dimids[dimids[0]] + m_dimids[dimids[1]] - m_mesh2d->face_nodes.size());  // only applicable if number of dimensions is two
                int total_edges = (int) (m_dimids[dimids[0]] * m_dimids[dimids[1]]);
...
int SUGRID::read_geometry_attributes(struct _geom_string* geom_strings, int i_var, std::string var_name, int topology_dimension)
{
...
int SUGRID::read_composite_mesh_attributes(struct _mesh_contact_string* mesh_contact_strings, int i_var, std::string var_name)
{
    int status = -1;
...
int SUGRID::read_network_attributes(struct _ntw_string* ntw_strings, int i_var, std::string var_name, size_t topology_dimension)
{
    int status = 1;
...
int SUGRID::get_attribute_by_var_name(int ncid, std::string var_name, std::string att_name, std::string* att_value)
{
    int status = -1;
    int i_var;
...
int SUGRID::read_grid_mapping(int i_var, std::string var_name, std::string grid_mapping_name)
{
    int status = -1;
    m_mapping->epsg = -1;
...
//------------------------------------------------------------------------------
int SUGRID::get_attribute(int ncid, int i_var, char * att_name, char ** att_value)
{
    size_t length = 0;
    int status = -1;
...
//------------------------------------------------------------------------------
int SUGRID::get_attribute(int ncid, int i_var, char * att_name, std::string * att_value)
{
    size_t length = 0;
    int status = -1;
...
//------------------------------------------------------------------------------
int SUGRID::get_attribute(int ncid, int i_var, std::string att_name, std::string * att_value)
{
    size_t length = 0;
    int status = -1;
...
//------------------------------------------------------------------------------
int SUGRID::get_attribute(int ncid, int i_var, char * att_name, double * att_value)
{
    int status = -1;
...
//------------------------------------------------------------------------------
int SUGRID::get_attribute(int ncid, int i_var, char * att_name, int * att_value)
{
    int status = -1;
...
//------------------------------------------------------------------------------
int SUGRID::get_attribute(int ncid, int i_var, char * att_name, long * att_value)
{
    int status = -1;
...
//------------------------------------------------------------------------------
int SUGRID::get_dimension(int ncid, char * dim_name, size_t * dim_length)
{
    int dimid;
    int status = -1;
...
//------------------------------------------------------------------------------
int SUGRID::get_dimension(int ncid, std::string dim_name, size_t * dim_length)
{
    int dimid;
    int status = -1;
...
//------------------------------------------------------------------------------
int SUGRID::get_dimension_var(int ncid, std::string var_name, size_t * dim_length)
{
...
std::vector<std::string>  SUGRID::get_dimension_names(int ncid, std::string var_name)
{
    int varid;
    int status = -1;
...
//------------------------------------------------------------------------------
std::vector<std::string> SUGRID::tokenize(const std::string& s, char c) {
    auto end = s.cend();
    auto start = end;
...
//
std::vector<std::string> SUGRID::tokenize(const std::string& s, std::size_t count)
{
...
char * SUGRID::strndup(const char *s, size_t n)
{
    char *result;
    size_t len = strlen(s);
...
double* SUGRID::permute_array(double* in_arr, std::vector<long> order, std::vector<long> dim)
{
    //
...
long SUGRID::get_index_in_c_array(long t, long l, long s, long xy, long time_dim, long layer_dim, long sed_dim, long xy_dim)
{
    Q_UNUSED(time_dim);
